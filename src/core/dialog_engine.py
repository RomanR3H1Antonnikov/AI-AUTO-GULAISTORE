...
⚠️ ЦЕНЫ — ПРИОРИТЕТ ИСТОЧНИКОВ (от высшего к низшему):

1. 🥇 ПОСЛЕДНЯЯ НАЗВАННАЯ В ДИАЛОГЕ ЦЕНА (наивысший приоритет)
   Если в истории переписки уже была названа конкретная сумма за этот товар — это и есть текущая цена.
   Используй именно её как базу при торге, скидках, подтверждениях — даже если она отличается от каталога.
   Это значит, что менеджер вручную скорректировал цену — его слово важнее каталога.
   ❌ НЕ ВОЗВРАЩАЙСЯ к цене каталога, если в диалоге уже фигурировала другая сумма.

2. 🥈 КАТАЛОГ (если цена в диалоге ещё не называлась)
   Если в переписке конкретной суммы ещё не было — бери цену из каталога выше.
   При расхождении с тем, что называл ранее: «Уточню — цена [X] ₽.»

3. 🥉 «ЦЕНА УТОЧНЯЕТСЯ» — если владелец уже назвал сумму в переписке, подтверди именно её.
...
class DialogEngine:
    ...
    async def reformulate_owner_reply(
        ...
        prompt = (
            ...
            "Задача: переформулируй ТОЛЬКО ответ владельца в короткое дружелюбное сообщение покупателю (1–2 предложения, без Markdown, на «вы»).\n\n"
            "Критически важные правила:\n"
            f"{opening_rule}"
            "• Используй ИСКЛЮЧИТЕЛЬНО то, что сказал владелец. Не добавляй ничего из контекста.\n"
